...
```{r}
m_2 <- stan_lmer(height ~ rain + (1 + rain | grp), dat)

# model summary
summary(m_2)

# rhat
which(rhat(m_2) > 1.1) # all good

# posterior predictive checks
pp_check(m_2, nreps = 25) # seems better than first model

# plot the predicted regression lines
newdat <- expand.grid(rain = seq(-2, 2, length.out = 10), grp = letters[1:n_grp])

pred <- posterior_predict(m_2, newdata = newdat)
## get median and 90 CrI
pred_sum <- apply(pred, 2, function(x) quantile(x, probs = c(0.1, 0.5, 0.9)))
newdat$height <- pred_sum[2,]
newdat$LCI <- pred_sum[1,]
newdat$UCI <- pred_sum[3,]
...
ggplot(newdat, aes(x=rain,y=height, group = grp)) +
  geom_ribbon(aes(fill=grp, ymin=LCI,ymax=UCI), alpha = 0.1) +
  geom_path(aes(color=grp)) +
  geom_point(data=dat, aes(color = grp)) # much better
...
loo_compare(ll_1, ll_2) # varying slope model clearly better
```
...
```{r}
brm_form4 <- brmsformula(biomass ~ nutrient, sigma ~ nutrient + I(nutrient ** 2))
m_4 <- brm(brm_form4, dat_2)
...
# check posterior predictive distribution
brms::pp_check(m_4, type = "dens_overlay", nsamples = 25)

# plot 
mm <- marginal_effects(m_4, method = "predict")
plot(mm, points = TRUE)

# derive loo for the two models and compare them
brms::loo(m_3, m_4, reloo = TRUE) # model m_4 much better
```
...
```{r}
m_6 <- brm(biomass ~ 1, data = dat_3, autocor = cor_ar(formula = ~ time, p = 2))

# usual checks ....

# compare the models
brms::loo(m_5, m_6) # little indication for th eneed to include a lag of 2
